// =============================================================================
//  vcm-combined  —  keyboard/ 의 위치별 클릭 세기 + vcm-force/ 의 위치별 임계힘
// -----------------------------------------------------------------------------
//  같은 위치값(keyAt()의 edge)을 두 곳에 동시에 먹인다. 세 스케치의 관계:
//
//    keyboard/    : 진폭만.  엣지에서 클릭이 약해진다. 임계힘은 어디서나 F_PEAK.
//                   -> "가장자리는 흐릿한 키"
//    vcm-force/   : 임계힘만. 엣지에서 더 세게 눌러야 넘어간다. 클릭 세기는 동일.
//                   -> "가장자리는 뻣뻣한 키"
//    vcm-combined : 둘 다.   엣지는 더 세게 눌러야 하는데 돌아오는 클릭은 약하다.
//                   -> "가장자리는 뻣뻣하고 흐릿한 키"
//
//  두 효과가 같은 방향으로 겹친다는 걸 알고 써야 한다. 엣지에서 임계힘 3배 +
//  진폭 0.1배면 "힘은 3배 드는데 손맛은 1/10"이라 중심으로 손가락을 몰아넣는
//  압력이 아주 세다. 각각 F_EDGE_SCALE / KEY_GAIN_MIN 으로 독립적으로 줄일 수
//  있으니, 어느 쪽이 실제로 효과를 내는지 보려면 한쪽씩 1.0 / 1.0f로 꺼 가며
//  비교하는 게 맞다 (그 두 극단이 각각 keyboard/ 와 vcm-force/ 다).
//
//  중심에는 EDGE_DEADBAND(약 ±3mm)의 스위트스팟이 있어서 그 안은 전부 중심과
//  똑같다 — 임계힘도 F_PEAK 그대로, 클릭도 최대 세기. 접촉 중심은 겨냥해도
//  1~2mm 흔들리므로 이게 없으면 평범한 타건이 전부 무겁고 흐려진다.
//  위치 의존은 그 바깥 밴드에만 걸린다. EDGE_DEADBAND를 0으로 두면 진폭 곡선이
//  keyboard/ 와 정확히 같아진다.
//
//  위치 의존을 거는 지점은 셋 (자세한 설명은 아래 튜너블 주석):
//    1) 임계힘   F_PEAK/F_VALLEY/F_END 에 forceScale(edge) 배수
//    2) 진폭     최종 구동값에 ampGain(edge) 곱  <- keyboard/ 에서 가져온 축
//    3) 프리로드 Z_PEAK/Z_VALLEY/Z_END 를 Z_EDGE_PRELOAD*edge 만큼 통째로 상승
//               (지금은 0 = 끔. 중심 Z_END가 듀티 상한을 쓰고 있어서다)
//  (2)와 (3)은 둘 다 "손가락이 받는 힘"을 건드리지만 방향이 반대다. (2)는 엣지의
//  클릭을 줄이고 (3)은 엣지의 지속 압력을 늘린다. 같이 켜면 서로 상쇄되니
//  한 번에 하나만 쓰는 게 좋다.
//
...
//                스위트스팟 테두리가 벽처럼 느껴진다.
// KEY_GAIN_MIN : 키캡 경계에서의 클릭 세기(중심=1.0). keyboard/ 와 같은 0.1이
//                기본. 1.0으로 두면 진폭 축이 꺼져서 vcm-force/ 와 같아진다.
// AMP_CURVE    : 진폭 감쇠 곡선의 지수. keyboard/ 와 같은 2.0이 기본
//                (중심 근처는 완만하고 가장자리에서 급격히 죽는다).
//                1.0으로 두면 선형.
...
// 유효 거리 -> 클릭 진폭 배수. 스위트스팟 안 1.0, 키캡 경계 KEY_GAIN_MIN.
// keyboard/ 의 keyAt()이 하던 일을 그대로 가져왔다. 거기서는 데드밴드가 없어서
// 중심을 벗어나는 즉시 감쇠가 시작됐다.
...
float sideDrive(SideState* s, const int idx[SIDE_SLOTS],
  ...
  for (int k = 0; k < SIDE_SLOTS; k++) {
    ...
    // 클릭 세기가 도중에 변하지 않는다. 임계힘 래치(heldEdge)와 같은 이유다.
    bool  live  = s->c[k].buttonState ? (s->c[k].heldKey >= 0) : onKey;
    float amp   = live ? ampGain(s->c[k].buttonState ? s->c[k].heldEdge : edge) : 0.0f;
    ...
    if (!prevBS && s->c[k].buttonState) {
      ...
      Serial.print("\tgain="); Serial.print(ampGain(edge), 2);
...
void setup() {
  ...
  Serial.print(" -> "); Serial.print((int)(F_PEAK * F_EDGE_SCALE));
  Serial.print(" (x"); Serial.print(F_EDGE_SCALE, 1); Serial.print(", curve ");
  Serial.print(F_EDGE_CURVE, 1); Serial.print(")   |   클릭 진폭 1.00 -> ");
  Serial.print(KEY_GAIN_MIN); Serial.print(" (curve ");
  Serial.print(AMP_CURVE, 1); Serial.println(") ===");
