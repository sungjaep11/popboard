...
struct ContactState {
  ...
  // 버클링 순간의 키 중심으로부터의 거리(0~1)를 같이 래치한다. 클릭 세기가
  // 여기서 나온다. heldKey와 같은 이유다: 누른 채 미끄러지면 진폭이 도중에
  // 바뀌어서 손가락이 누르고 있는 키가 점점 물러지거나 세지는 게 느껴진다.
  // 한 번 버클링한 키는 뗄 때까지 "누른 순간의 세기"를 유지해야 한다.
  float heldEdge;
};
